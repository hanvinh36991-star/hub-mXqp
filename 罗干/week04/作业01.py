...
print("dataset_df[subject]",dataset_df['subject'].values[:500])
# 初始化 LabelEncoder，用于将文本标签转换为数字标签
lbl = LabelEncoder()
# 拟合数据并转换前500个标签，得到数字标签
print("dataset_df[1].values[:500]",dataset_df['subject'].values[:500])
labels = lbl.fit_transform(dataset_df['subject'].values[:500])
# 提取前500个文本内容
...
model = BertForSequenceClassification.from_pretrained('google-bert/bert-base-chinese', num_labels=17)
