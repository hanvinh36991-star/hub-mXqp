...
# 加载和预处理数据
dataset_df = pd.read_csv("/Users/fancheng/project/homework/week04/Simplified_Chinese_Multi-Emotion_Dialogue_Dataset.csv", sep=",", header=0)
...
test_text = "你是在怀疑我嘛？"
test_encoding = tokenizer(test_text, truncation=True, padding=True, max_length=64, return_tensors="pt")

##因为device 是cpu，所以要将模型和输入数据都移动到cpu
...
trainer.model.eval()
with torch.no_grad():
    outputs = trainer.model(**test_encoding)
    logits = outputs.logits
    predicted_label_idx = torch.argmax(logits, dim=1).item()
    predicted_label = lbl.inverse_transform([predicted_label_idx])[0]
