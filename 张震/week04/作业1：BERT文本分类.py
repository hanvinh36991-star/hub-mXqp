...
labels = lbl.fit_transform(dataset_df["label"].values[:1000])
# 提取前500个文本内容
texts = list(dataset_df["review"].values[:1000])
...
if best_model_path:
    ...
    model.load_state_dict(torch.load("../models/weights/bert.pt"))
    model.to(device)


def model_for_bert(text):
    test_encoding = tokenizer(list(text), truncation=True, padding=True, max_length=64)
    test_dataset = NewsDataset(test_encoding, [0] * len(text))
    ...
    print(pred)
    classify_result = [[0, 1][x] for x in pred]
    return classify_result

# trainer 是比较简单，适合训练过程比较规范化的模型
# 如果我要定制化训练过程，trainer无法满足

model_for_bert("新楼环境还是可以的,不过貌似没什么人,中餐厅的装修还是比较陈旧.服务周到,值得下次再去")
